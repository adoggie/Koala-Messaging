...
class EventListener(tce.RpcConnectionEventListener):
	...
	def onDataPacket(self, conn, m):
		...
		if conn.getRecvedMessageCount() == 1:
			error = lemon.base.ErrorDefs.SUCC

			token = m.extra.getValue('__token__')
			if token:
				auth = lemon.encrypt.decryptUserToken(token)
				if auth:
					if self.isTokenSessionExpired(auth):
						error = lemon.base.ErrorDefs.UserTokenSessionExpired
					else:
						userid = auth.user_id
						conn.setUserId(int(userid))
						#self.server.onUserOnline(userid,'',conn) #todo. commented temperily
						print 'user check passed, user:',auth.user_id,auth.user_name
				else:
					error = lemon.base.ErrorDefs.UserTokenInvalid
			else:
				error = lemon.base.ErrorDefs.UserTokenInvalid
...
class TerminalGatewayServer(ITerminalGatewayServer):
	def __init__(self):
		...
		self.prxUserEventListener = service.lemon_impl.IUserEventListenerPrx.createWithEpName('mq_user_event_listener')
...
if __name__ == '__main__':
	...
	p ='gwserver.py -name gwserver2 -listen socket_gateway_1,mq_gateway_1,mq_gateway_broadcast -loopback mq_messageserver#mq_gateway_1'
	print 'start <<SOCKET-SEVER>>..'
